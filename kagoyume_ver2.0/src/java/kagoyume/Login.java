        ...
        try {
            ...
            if(request.getParameter("set") != null && request.getParameter("set").equals("login")){
                ...
                if(UserDataDAO.getInstance().CheckUserData(udd)){
                    ...
                    lb.setPreviosPage(request.getParameter("previousPage"));//このパラメータ値はlogin.jspから
                    request.setAttribute("loginInfo", lb);
                    session.removeAttribute("CartItem");//別のユーザーから違うユーザーに変えた場合、既にsession("CartItem")に情報が格納されているのでremove
                    session.setAttribute("loginUserData", loginUserData);//ログインに成功したユーザー情報を格納
                    request.getRequestDispatcher("Cart").forward(request, response);//session("CartItem")に新しくログインした人物のカート情報を挿入する為に一旦Cart.javaに移動
                    return;
                } else {
