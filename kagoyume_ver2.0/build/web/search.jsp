...
<%
    ...
    if(request.getParameter("pageNum") != null){
        pageNum = request.getParameter("pageNum");//ページ番号のパラメータをStringに代入
    } else {
        pageNum = "1";
    }
    int ipageNum = Integer.parseInt(pageNum);//ページ番号をint変換
    int lastNum = Integer.parseInt(pageNum + 0);//ページ内の最後の番号 + 1の値を入れる(例:3ページの最後 + 1は30)
    int firstNum;//ページ内の最初の番号を入れる（例:2ページの1番目は10）
    if(ipageNum - 1 == 0){
        firstNum = 0;
    }else{
        firstNum = (ipageNum - 1) * 10;
    }
    int pageCount = 0;
    if(sr.getHitCount() <= 10){
        pageCount = 1;//YahooApiから返ってきた商品個数が10個以下の場合1
    } else if(sr.getHitCount() >= 11 && sr.getHitCount() <= 20){
        pageCount = 2;//YahooApiから返ってきた商品個数が11個以上20個以下の場合2
    } else if(sr.getHitCount() >= 21 && sr.getHitCount() <= 30){
        pageCount = 3;//YahooApiから返ってきた商品個数が21個以上30個以下の場合3
    } else if(sr.getHitCount() >= 31 && sr.getHitCount() <= 40){
        pageCount = 4;//YahooApiから返ってきた商品個数が31個以上40個以下の場合4
    } else if(sr.getHitCount() >= 41 && sr.getHitCount() <= 50){
        pageCount = 5;//YahooApiから返ってきた商品個数が41個以上50個以下の場合5
    }
%>
<%-- 
    Document   : search
...
<html>
    ...
    <body>
        ...
        <nav class="navbar navbar-default navbar-fixed-top">
            <div class="container-fluid">
                <div class="navbar-header">
                    <button type="button" class="navbar-toggle" data-toggle="collapse" data-target="#navBootstrap">
            ...
            <div class="row">
                <form action="Search" method="GET" class="form-horizontal">
                    ...
                    <div class="collapse" id="collapse">
                        <div class="form-group container">
                            <label class="col-sm-3 control-label" for="category" style="font-size: 110%; font-family: sans-serif">商品検索：</label>
                            <div class="col-sm-8">
        ...
        <div class="container-fluid">
            <ol class="breadcrumb">
                <li><a href="Top"><i class="fas fa-home"></i>かごゆめ</a></li>
                <%if(sr.getCategoryName().size() == 2){%>
                <li><a href="<% out.print("Search?category_id=" + sr.getB_cateId() + "&pageNum=1");%>"><%=sr.getCategoryName().get(0)%></a></li>
                <li class="active"><%=sr.getCategoryName().get(1)%></li>
                <%} else {%>
                <li class="active"><%=sr.getCategoryName().get(0)%></li>
                <%}%>
            </ol>
            <div class="row">
                ...
                <div class="contents col-sm-9 col-xs-12">
                    <%if(sr.getHitCount() != 0){%>
                    <%if(!sr.getKeyword().equals("")){%><p style="font-size: 120%; font-family: serif"><strong>「<span style="color: red">"<%=sr.getKeyword()%>"</span>」の検索結果　<%if(!sr.getHighPrice().equals("")){%>最高金額設定:<%=sr.getHighPrice()%>円<%}%>　<%if(!sr.getLowPrice().equals("")){%>最低金額設定:<%=sr.getLowPrice()%>円<%}%></strong></p><%}%>
                    <nav class="navbar navbar-default">
                        <div class="container-fluid">
                            <div class="collapse navbar-collapse">
                                <form action="Search" method="GET" class="form-inline">
                                    <div class="form-group">
                                        <button type="submit" class="btn btn-default navbar-btn <%if(sr.getSort().equals("-score")){out.print("active");}%>" name="sort" value="-score">おすすめ順</button>
                                        <button type="submit" class="btn btn-default navbar-btn <%if(sr.getSort().equals("+price")){out.print("active");}%>" name="sort" value="+price">価格安い順</button>
                                        <button type="submit" class="btn btn-default navbar-btn <%if(sr.getSort().equals("-price")){out.print("active");}%>" name="sort" value="-price">価格高い順</button>
                                        <input type="hidden" name="sortChange" value="true">
                                        <input type="hidden" name="pageNum" value="1">
                                    </div>
                                </form>
                            </div>
                    ...
                    <p class="text-right"><%out.print("--該当数" + sr.getHitCount() + "件--");%></p>
                    <table class="table table-hover">
                        <thead>
                            <tr><th>サムネイル画像</th><th>商品名</th><th>値段</th></tr>
                        </thead>
                        <tbody>
                        <%if(sr.getHitCount() < lastNum){%>
                            <%for(int number = firstNum; number < sr.getHitCount(); number++){%>
                    ...
                    <ul class="pagination">
                    <li>
                        <a <%if(pageNum.equals("1")){out.print("href=search.jsp?pageNum=1");}else{out.print("href=search.jsp?pageNum=" + String.valueOf(Integer.parseInt(pageNum) - 1));}%> aria-label="前のページへ">
                            <span aria-hidden="true">«</span>
                        </a>
                    </li>
                    <li <%if(pageNum != null && pageNum.equals("1")){%>class="active"<%}%>><a href="search.jsp?pageNum=1">1</a></li>
                    <%if(sr.getHitCount() > 10){%><li <%if(pageNum != null && pageNum.equals("2")){%>class="active"<%}%>><a href="search.jsp?pageNum=2">2</a></li><%}%>
                    <%if(sr.getHitCount() > 20){%><li <%if(pageNum != null && pageNum.equals("3")){%>class="active"<%}%>><a href="search.jsp?pageNum=3">3</a></li><%}%>
                    <%if(sr.getHitCount() > 30){%><li <%if(pageNum != null && pageNum.equals("4")){%>class="active"<%}%>><a href="search.jsp?pageNum=4">4</a></li><%}%>
                    <%if(sr.getHitCount() > 40){%><li <%if(pageNum != null && pageNum.equals("5")){%>class="active"<%}%>><a href="search.jsp?pageNum=5">5</a></li><%}%>
                    <li>
                        <a <%if(ipageNum < pageCount){out.print("href=search.jsp?pageNum=" + String.valueOf(Integer.parseInt(pageNum) + 1));} else {out.print("href=search.jsp?pageNum=" + pageNum);}%> aria-label="次のページへ">
                            <span aria-hidden="true">»</span>
                        </a>
                    </li>
                    </ul>
                    ...
                    <%} else {%>
                    <h1 style="font-size: 120%; font-family: serif">検索キーワード「"<%=sr.getKeyword()%>"」に該当する商品はありませんでした　<%if(!sr.getHighPrice().equals("")){%>最高金額設定:<%=sr.getHighPrice()%>円<%}%>　<%if(!sr.getLowPrice().equals("")){%>最低金額設定:<%=sr.getLowPrice()%>円<%}%></h1>
                    <%}%>
                </div>
            </div>
        </div>
