...
public class UserDataDAO {
    ...
    /**
     * ログインで記入したメールアドレス、パスワードがデータベース上に存在するかチェックし、存在すればtrue。しなければfalseで返す
     * @param udd
     * @return boolean
     * @throws SQLException 
     */
    public boolean CheckUserData(UserDataDTO udd) throws SQLException {
        ...
        try {
            con = DBManager.getConnection();
            ps = con.prepareStatement("SELECT name, password FROM user_t WHERE mail = ? AND password = ? AND deleteFlg = 0");
            ps.setString(1, udd.getMail());
            ps.setString(2, udd.getPassword());
    ...
    public UserDataDTO LoginUserData(UserDataDTO udd) throws SQLException{
        ...
        try{
            con = DBManager.getConnection();
            ps = con.prepareStatement("SELECT * FROM user_t WHERE mail = ? AND password = ? AND deleteFlg = ?");
            ps.setString(1, udd.getMail());
            ps.setString(2, udd.getPassword());
            ps.setInt(3, 0);
            ResultSet rs = ps.executeQuery();
    ...
    /**
     * 新規会員登録で記入されたメールアドレスが既に存在する場合true、存在しない場合falseを返す
     * @param ud
     * @return
     * @throws SQLException 
     */
    public boolean CheckRegisteredUser(UserData ud) throws SQLException{
        Connection co = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        try{
            co = DBManager.getConnection();
            ps = co.prepareStatement("SELECT * FROM user_t WHERE mail = ?");
            ps.setString(1, ud.getMail());
            rs = ps.executeQuery();
            return rs.next();
        } catch(SQLException e) {
            System.out.println(e.getMessage());
